...
export function WeddingLetterBox({ slug, initialLetters, onPopupOpenChange }: WeddingLetterBoxProps) {
    ...
    return (
        <div className="space-y-4">
            {!showForm ? (
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-amber-100 p-4 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                            ...
                            className="p-1 rounded-lg hover:bg-amber-50 text-gray-400"
                        >
                            <X className="w-5 h-5" />
                        </button>
                    </div>
                    <form onSubmit={handleSubmit} className="space-y-3">
                        <input
                            ...
                            maxLength={50}
                            className="w-full px-4 py-2 rounded-lg border border-amber-200 focus:outline-none focus:ring-2 focus:ring-amber-300 bg-white/80"
                            required
                        />
                        <textarea
                            ...
                            maxLength={1000}
                            rows={4}
                            className="w-full px-4 py-2 rounded-lg border border-amber-200 focus:outline-none focus:ring-2 focus:ring-amber-300 bg-white/80 resize-none"
                            required
                        />
            ...
            {letters.length === 0 ? (
                <div className="text-center py-12 bg-white/60 backdrop-blur-sm rounded-xl border border-amber-100">
                    <Mail className="w-12 h-12 text-amber-300 mx-auto mb-3" />
                    <p className="text-gray-500">Chưa có lời chúc nào</p>
                    <p className="text-sm text-gray-400 mt-1">Hãy là người đầu tiên gửi lời chúc!</p>
                </div>
            ) : (
                    ...
                    {letters.map((letter) => (
                        <div
                            ...
                            className={`bg-white/80 backdrop-blur-sm rounded-xl border ${
                                isLocked(letter) ? "border-gray-200 opacity-75" : "border-amber-100"
                        ...
                        >
                            {isLocked(letter) ? (
                                <div className="p-4 text-center">
                                    ...
                                    <p className="text-sm text-gray-500 mb-1">Lời chúc này sẽ được mở khóa vào</p>
                                ...
                                <>
                                    <div className="p-4">
                                        <div className="flex items-start justify-between mb-2">
                                            <div>
                                                <h4 className="font-medium text-amber-900">{letter.sender || "Ẩn danh"}</h4>
                                    ...
                                    {letter.replies.length > 0 && (
                                        <div className="border-t border-amber-50 bg-amber-50/30">
                                            <button
                                                ...
                                                className="w-full flex items-center justify-between px-4 py-2 text-sm text-amber-600 hover:bg-amber-50/50 transition-colors"
                                            >
                                                    ...
                                                    {letter.replies.map((reply) => (
                                                        ...
                                                        >
                                                            <div className="flex items-start justify-between mb-1">
                                                                <div>
                                                                    <span className="font-medium text-amber-900 text-sm">{"Phản hồi"}</span>
                                    ...
                                    {replyingTo === letter.id ? (
                                        <div className="border-t border-amber-100 bg-amber-50/50 p-3">
                                            <form onSubmit={(e) => handleReply(e, letter.id)} className="space-y-2">
                                                <input
                                                    ...
                                                    maxLength={50}
                                                    className="w-full px-3 py-1.5 rounded-lg border border-amber-200 focus:outline-none focus:ring-2 focus:ring-amber-300 text-sm bg-white/80"
                                                    required
                                                />
                                                <textarea
                                                    ...
                                                    maxLength={1000}
                                                    rows={2}
                                                    className="w-full px-3 py-1.5 rounded-lg border border-amber-200 focus:outline-none focus:ring-2 focus:ring-amber-300 text-sm resize-none bg-white/80"
                                                    required
                                                />
                                                <div className="flex gap-2">
                                                ...
                                                <div className="flex gap-2">
                                                    <button
                                                        type="submit"
                                                        disabled={isSubmitting}
                                                        className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg bg-amber-400 text-white text-sm font-medium hover:bg-amber-500 transition-colors disabled:opacity-50"
                                                    >
                                                        {isSubmitting ? (
                                                    ...
                                                    <button
                                                        ...
                                                        className="px-3 py-1.5 rounded-lg border border-amber-200 text-amber-600 text-sm hover:bg-amber-50 transition-colors"
                                                    >
                                                        Hủy
                                                    </button>
                                                </div>
                                        ...
                                        <button
                                            ...
                                            className="w-full flex items-center justify-center gap-1 py-2 border-t border-amber-100 text-amber-600 text-sm hover:bg-amber-50/50 transition-colors"
                                        >

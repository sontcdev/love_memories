...
// GradGroupTemplateV2 — bản giữ nguyên implementation mới (UX roadmap) của GradGroupTemplate.
// GradGroupTemplate.tsx đã được rollback về đúng phiên bản trên nhánh deploy, nên mọi
...
import { Image as ImageIcon, Mail, ChevronLeft, ChevronRight, Settings, Sparkles, X, Target, Users, MapPin, Navigation, Calendar, Flag } from "lucide-react";
...
interface GradGroupTemplateV2Props {
...
export function GradGroupTemplateV2({ data, slug }: GradGroupTemplateV2Props) {
    ...
    const subThemeDarkBg =
        subTheme === "station" ? "#05040a" : subTheme === "scrapbook" ? "#1c1611" : "#0f0a07";

    const { isDark, toggle: handleThemeToggle } = useThemeToggle({
        slug,
        darkBg: subThemeDarkBg,
        lightBg: data.config?.background_color || "#3a2213",
    });
            ...
            <div className="fixed top-0 left-0 right-0 z-30 flex items-center justify-between px-4 py-2">
                ...
                <div className="flex items-center gap-2">
                    <ThemeToggleButton
                        ...
                        className={`p-2.5 rounded-full shadow-lg transition-all hover:scale-110 ${theme.cardBg} ${theme.textColor} backdrop-blur-md`}
                    />

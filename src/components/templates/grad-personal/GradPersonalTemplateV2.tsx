...
// GradPersonalTemplateV2 — bản giữ nguyên implementation mới (UX roadmap) của GradPersonalTemplate.
// GradPersonalTemplate.tsx đã được rollback về đúng phiên bản trên nhánh deploy, nên mọi
...
import { Calendar, Image as ImageIcon, Mail, ChevronLeft, ChevronRight, Settings, Sparkles, X, Target, Award, GraduationCap } from "lucide-react";
...
interface GradPersonalTemplateV2Props {
...
export function GradPersonalTemplateV2({ data, slug }: GradPersonalTemplateV2Props) {
            ...
            <div className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-2 border-b ${isDark ? "bg-[#1a1612]/95 border-amber-900/30" : "bg-white/95 border-amber-200 shadow-sm"} backdrop-blur-sm`}>
                ...
                <div className="flex items-center gap-2">
                    <ThemeToggleButton
                        ...
                        className={`p-2 rounded-lg transition-all hover:scale-110 ${isDark ? "text-yellow-400 hover:bg-amber-900/30" : "text-amber-700 hover:bg-amber-100"}`}
                    />

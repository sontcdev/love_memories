...
// EditGradGroupProfileFormV2 — bản giữ nguyên implementation mới (toast, auto-save, undo/redo…).
// EditGradGroupProfileForm.tsx đã rollback về đúng phiên bản trên nhánh deploy và chỉ phục vụ
...
import { useFormAutoSave } from "./useAutoSave";
...
export function EditGradGroupProfileFormV2({ slug, initialData, isDark = false, onSuccess }: EditGradGroupProfileFormV2Props) {
    ...
    const {
        ...
        // hưởng nút "Lưu thay đổi hồ sơ nhóm": `handleSubmit` vẫn validate như trước.
        mode: "onChange",
        defaultValues: {
     ...
     *
     * Cả nút "Lưu thay đổi hồ sơ nhóm" và tự động lưu đều đi qua đây, nên không có
     * chỗ nào gọi `updateLinkProfile` lần thứ hai — hai luồng không thể lệch nhau về
     * payload (ảnh, chủ đề, thành viên, câu đố, cột mốc) hay về cách xử lý lỗi.
     */
    const persist = useCallback(
        ...
        [slug, groupAvatar, theme, members, quiz, quizBadges, goals]
    );

    /**
     * Phần lớn nội dung của hồ sơ nhóm KHÔNG nằm trong react-hook-form: ảnh đại diện,
     * chủ đề, danh sách thành viên, câu đố và bản đồ chặng đường đều là state riêng.
     * Nếu chỉ nghe `watch()` thì thêm cả một thành viên cũng không được tự động lưu —
     * đúng loại thay đổi tốn công nhất và dễ mất nhất.
     *
     * So sánh theo nội dung với dữ liệu lúc mở trang (giữ trong ref để mốc so sánh
     * không đổi giữa các lần render). `quiz_badges` không có UI sửa nên không cần xét.
     */
    const initialExtras = useRef(
        JSON.stringify({
            groupAvatar: castData.group_avatar || "",
            theme: castData.theme || "caravan",
            members: castData.members || [],
            quiz: castData.quiz || [],
            goals: castData.goals || [],
        })
    );
    const extrasDirty =
        JSON.stringify({ groupAvatar, theme, members, quiz, goals }) !== initialExtras.current;
     ...
     * - `!uploadingGroupAvatar && uploadingAvatarId === null`: đang tải ảnh tập thể
     *   hoặc ảnh của một thành viên thì URL trong state còn là ảnh cũ, ghi lúc này sẽ
     *   đè mất ảnh vừa tải lên.
     *
     * KHÔNG có hoàn tác/làm lại ở form này: chỉ 4 ô chữ nằm trong react-hook-form,
     * còn thành viên/câu đố/cột mốc là các state riêng với thao tác thêm-xóa của
     * riêng chúng. Một ngăn xếp undo chỉ quản được 4 ô kia sẽ nói dối người dùng —
     * sửa tên thành viên rồi bấm "Hoàn tác" lại thấy ô Tên nhóm nhảy về giá trị cũ.
     */
    const autoSave = useFormAutoSave({
        watch: () => ({ ...watch(), groupAvatar, theme, members, quiz, goals }),
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Trạng thái tự động lưu, đặt ngay trên nút Lưu */}
